...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (insertError) {
        console.error('Error creating restriction reason:', insertError);
        return res.status(500).json({ 
          error: 'Failed to create restriction reason',
          details: insertError 
        });
      }
      ...
      return res.status(201).json({ 
        success: true, 
        message: 'Restriction reason created successfully',
        reason: newReason 
      });
    }
    ...
    if (req.method === 'PUT') {
      ...
      if (updateError) {
        console.error('Error updating restriction reason:', updateError);
        return res.status(500).json({ 
          error: 'Failed to update restriction reason',
          details: updateError 
        });
      }
      ...
      return res.status(200).json({ 
        success: true, 
        message: 'Restriction reason updated successfully',
    ...
    if (req.method === 'DELETE') {
      ...
      if (soft_delete) {
        ...
        if (deactivateError) {
          console.error('Error deactivating restriction reason:', deactivateError);
          return res.status(500).json({ 
            error: 'Failed to deactivate restriction reason',
        ...
        return res.status(200).json({ 
          success: true, 
          message: 'Restriction reason deactivated successfully',
        ...
        if (deleteError) {
          console.error('Error deleting restriction reason:', deleteError);
          return res.status(500).json({ 
            error: 'Failed to delete restriction reason',
            details: deleteError 
          });
        }
        ...
        return res.status(200).json({ 
          success: true, 
          message: 'Restriction reason deleted permanently' 
        });
      }
    }
  } catch (error) {
    console.error('Server error in manage-restriction-reason:', error);
    return res.status(500).json({ 

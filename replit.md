...
- **Professional Timestamp Editor with Multi-Select Filters (Nov 17, 2025):** Enhanced `/admin/edit-user-timestamps` page with professional bulk update functionality and advanced filtering capabilities. Implemented multi-select filters for months (visual chips), years (visual chips), combined months & years, specific dates with removable chips, and date range filtering. Added date-only vs datetime toggle checkbox to set timestamps to midnight (00:00:00) when time component is not needed. All filter state is scoped to the bulk update modal only, keeping the main page unaffected and always showing all timestamp fields for manual editing. Filters properly include null/empty timestamps when "All Timestamps" is selected, enabling easy backfilling of missing timestamps. Modal automatically resets filters on close for clean user experience.
- **Admin UI Consistency Improvements (Nov 16, 2025):** Enhanced UI consistency across admin pages by updating `/admin/approve-applications` to match the polished design of `/admin/admin-loans`. Added stats cards showing total, pending, approved, rejected, and under review application counts. Implemented tab-based filtering for application status with client-side filtering for accurate statistics. Added comprehensive search (name, email, phone) and date range filtering capabilities. Updated card layout with modern gradient buttons, hover effects, and responsive clamp() font sizes. Added update/confirmation modal functionality to `/admin/manage-crypto-deposits` page with dedicated Update button for pending, confirmed, and processing deposits. Modal includes fields for amount, transaction hash, confirmations, status, rejection reason, and admin notes, with visual feedback for deposit completion actions.

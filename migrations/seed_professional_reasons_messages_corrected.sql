-- ============================================================================
-- SEED DATA: Professional Restriction Reasons & Display Messages
...
restriction_insert AS (
  ...
  SELECT 
    ...
    is_active
  FROM (
    SELECT 'ban_user' as action_type, 'Security' as category, 'Account compromised - unauthorized access detected from multiple locations and suspicious transactions initiated' as reason_text, be.security_email as contact_email, 'critical'::text as severity_level, true as requires_immediate_action, 1 as display_order, true as is_active FROM bank_emails be
    UNION ALL
...
-- ============================================================================
-- 3. INSERT DISPLAY MESSAGES FOR RESTRICTION REASONS
-- ============================================================================
...
SELECT
  ...
  is_active
FROM (
  SELECT 'unban_user' as action_type, 'Appeals' as category, 'User submitted successful appeal - identity verified and account restored per appeal decision' as reason_text, be.compliance_email as contact_email, 'high'::text as severity_level, false as requires_immediate_action, 1 as display_order, true as is_active FROM bank_emails be
  UNION ALL
...
-- ============================================================================
-- VERIFICATION: Check inserted data (uncomment to test)
-- ============================================================================

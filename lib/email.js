...
// SendPulse SMTP transporter
function createSendPulseTransporter() {
  if (!process.env.SENDPULSE_SMTP_HOST || !process.env.SENDPULSE_SMTP_USER || !process.env.SENDPULSE_SMTP_PASS) {
    return null;
  }
  
  return nodemailer.createTransport({
    host: process.env.SENDPULSE_SMTP_HOST,
    port: parseInt(process.env.SENDPULSE_SMTP_PORT, 10) || 465,
    secure: process.env.SENDPULSE_SMTP_PORT === '465',
    auth: {
...
// Send email via Brevo API (formerly Sendinblue)
async function sendViaBrevo({ to, subject, html, text, from }) {
  ...
  // Extract display name and email address from "Name <email@domain.com>" format
  let fromName = null;
  let fromEmail = from;
  ...
  if (!fromEmail.includes(`@${emailDomain}`)) {
    console.warn(`⚠️ FromAddress ${fromEmail} is not on verified domain. Using info@${emailDomain}`);
    fromEmail = `info@${emailDomain}`;
  }

  // Brevo email payload
  const emailPayload = {
    sender: {
      name: fromName || 'Oakline Bank',
      email: fromEmail
    },
...
async function sendViaResend({ to, subject, html, text, from }) {
  ...
  // Reconstruct from address with display name for Resend
...
function getEmailFrom(type = EMAIL_TYPES.DEFAULT) {
  ...
  switch (type) {
    case EMAIL_TYPES.WELCOME:
      return `${fromName} <${process.env.SMTP_FROM_WELCOME || process.env.SMTP_FROM || `info@${emailDomain}`}>`;
    case EMAIL_TYPES.UPDATES:
      return `${fromName} <${process.env.SMTP_FROM_UPDATES || process.env.SMTP_FROM || `info@${emailDomain}`}>`;
    case EMAIL_TYPES.CONTACT:
      return `${fromName} <${process.env.SMTP_FROM_CONTACT || process.env.SMTP_FROM || `contact-us@${emailDomain}`}>`;
    case EMAIL_TYPES.NOTIFY:
      return `${fromName} <${process.env.SMTP_FROM_NOTIFY || process.env.SMTP_FROM || `info@${emailDomain}`}>`;
    case EMAIL_TYPES.LOANS:
      return `${fromName} <${process.env.SMTP_FROM_LOANS || process.env.SMTP_FROM || `loans@${emailDomain}`}>`;
    case EMAIL_TYPES.SECURITY:
      return `${fromName} <${process.env.SMTP_FROM_SECURITY || process.env.SMTP_FROM || `info@${emailDomain}`}>`;
    case EMAIL_TYPES.VERIFY:
      return `${fromName} <${process.env.SMTP_FROM_VERIFY || process.env.SMTP_FROM || `info@${emailDomain}`}>`;
    case EMAIL_TYPES.CRYPTO:
      return `${fromName} <${process.env.SMTP_FROM_CRYPTO || process.env.SMTP_FROM || `crypto@${emailDomain}`}>`;
    default:
      return `${fromName} <${process.env.SMTP_FROM || `info@${emailDomain}`}>`;
  }
}
...
export async function sendEmail({ to, subject, text, html, type = EMAIL_TYPES.DEFAULT, from }) {
  ...
  // Import supabaseAdmin for logging
  let supabaseAdmin;
  try {
    const supabaseModule = await import('./supabaseAdmin.js');
    supabaseAdmin = supabaseModule.supabaseAdmin;
  } catch (importError) {
    console.warn('Could not import supabaseAdmin for email logging:', importError.message);
  }
  ...
  if (supabaseAdmin) {
    try {
      ...
      const { data: logEntry, error: logError } = await supabaseAdmin
        ...
        .insert({
          recipient_email: to,
          recipient_user_id: userData?.id || null,
          subject: subject,
          email_type: type,
          provider: 'pending',
          status: 'pending',
      ...
      console.warn('Failed to create email log entry:', logErr.message);
    }
  }
  ...
  try {
    ...
    if (process.env.BREVO_API_KEY) {
      try {
        result = await sendViaBrevo({ to, subject, html, text, from: fromAddress });
        providerUsed = 'Brevo';
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        result = await sendViaResend({ to, subject, html, text, from: fromAddress });
        providerUsed = 'Resend';
    ...
    if (primaryTransporter) {
      try {
        ...
        console.warn('⚠️ Primary SMTP failed, trying SendPulse fallback:', primaryError.message);
      }
    }

    // Try SendPulse SMTP as last resort
    const sendpulseTransporter = createSendPulseTransporter();
    if (sendpulseTransporter) {
      result = await sendViaSMTP({ 
        to, 
        subject, 
        html, 
        text, 
        from: fromAddress,
        transporterName: 'SendPulse',
        transporter: sendpulseTransporter
      });
      providerUsed = 'SendPulse';
      ...
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="padding: 40px 32px;">
          ...
          <p style="color: #64748b; font-size: 14px; line-height: 1.6; margin: 16px 0;">
            Please visit <a href="${process.env.NEXT_PUBLIC_SITE_URL || 'https://www.theoaklinebank.com'}" style="color: #1e40af;">${process.env.NEXT_PUBLIC_SITE_DOMAIN || 'www.theoaklinebank.com'}</a> to access your account.
          </p>

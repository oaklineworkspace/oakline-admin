...
async function sendViaResend({ to, subject, html, text, from }) {
  ...
  if (!fromAddress.includes(`@${emailDomain}`)) {
    console.warn(`⚠️ FromAddress ${fromAddress} is not on verified domain. Using info@${emailDomain}`);
    fromAddress = `info@${emailDomain}`;
  }

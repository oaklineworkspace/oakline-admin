...
function getEmailFrom(type = EMAIL_TYPES.DEFAULT) {
  ...
  switch (type) {
    ...
    case EMAIL_TYPES.WITHDRAWAL:
      return `${fromName} <${process.env.SMTP_FROM_WITHDRAWALS || process.env.SMTP_FROM || `withdrawals@${emailDomain}`}>`;
    default:
...
export async function sendEmail({ to, subject, text, html, type = EMAIL_TYPES.DEFAULT, from }) {
  ...
  console.log('📧 Available providers - Brevo:', !!process.env.BREVO_API_KEY, ', Resend:', !!process.env.RESEND_API_KEY, ', SMTP:', !!process.env.SMTP_HOST);
